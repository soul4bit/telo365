...
**Для текущей версии с аккаунтами использовать [application.md](application.md) и `install-app.sh`.** Ниже сохранена исходная инструкция для статического демо и настройки входного nginx. `install-site.sh` не применять к работающей серверной версии: он заменяет её конфигурацию nginx.

Требования первоначальной установки: Ubuntu 24.04+, Python 3.12+, sudo, SSH по ключу. Текущая версия дополнительно использует Node.js 24 и SQLite; установщик приложения настраивает их автоматически.
...
Интернет → 80.82.48.115:80/443 → прежний nginx (ВМ SoulCam)
                                ...
                                └─ telo365.ru → новая ВМ:8080 → Node.js:1435 → SQLite
```
