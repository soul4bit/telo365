...
export function Dialog({title,children,close,className}:{title:string;children:ReactNode;close:()=>void;className?:string}) {
  ...
  return <dialog ref={ref} className={className} aria-label={title} onCancel={close} onClick={e=>{if(e.target===ref.current)close()}}><div className="dialog-head"><h2>{title}</h2><button className="icon-button" aria-label="Закрыть" onClick={close}><X/></button></div>{children}</dialog>
}
