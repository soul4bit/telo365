...
## Принятый базовый шаблон

24.09.2026 владелец принял студию из выпуска `4496ddd` как базовый визуальный шаблон TELO365. Полировка окружения squat на этом завершается. Принятие оформления не меняет `specialistTechniqueReview: pending` и `verified: false`.

Для будущих упражнений повторно использовать `FunctionalStudio` и `StudioLighting`, исходную планировку, материалы, позиции оборудования, брендинг и направление света. Надпись `TELO365.RU` оставить в текущем размере и контрасте; дополнительное ослабление не требуется. Камера `3/4` и скорость `1×` остаются начальными настройками интерфейса.

Одинаковый масштаб фиксируется параметрами съёмки: общие радиус, высота, FOV, target и соотношение сторон для всех четырёх ракурсов одного упражнения и обоих полов. Запрещены индивидуальный zoom, crop или масштабирование скелета ради заполнения кадра. Разница проекции тела при повороте камеры естественна; её не надо компенсировать увеличением отдельных роликов. В течение цикла камера неподвижна.

Эталон: восемь текущих WebM/PNG squat, контрольные листы `artifacts/studio-quality-review/{male|female}-contact-sheet.png` и параметры камер ниже. До повторного экспорта сравнивать фронтальный, боковой, задний и 3/4 виды с этим эталоном. Перестановка окружения или света требует пересмотра шаблона, а не частной правки одного ракурса.

Следующий этап — проверка шаблона на других типах движения, без автоматического создания или публикации упражнений:

| Движение | Что проверить в том же помещении |
| --- | --- |
| Push-up | Всё вытянутое тело, кисти и стопы, коврик, нижняя точка без перекрытия controls |
| Plank | Полная длина тела, читаемый наклон корпуса, устойчивый кадр статического удержания |
| Glute bridge | Положение лёжа, подъём таза и крайние точки коленей без crop |
| Lunge | Продольная амплитуда шага, обе стопы, верхняя и нижняя точки |
| Dumbbell row | Наклон корпуса и полный путь гантели, при необходимости отдельное оборудование упражнения |

Сначала проверять существующий набор камер без изменений. Если напольное или вытянутое движение не помещается либо становится слишком мелким, сохранить контрольные кадры и измерения; только затем согласовывать общий профиль съёмки для такого класса движений. Не объявлять универсальность squat-камеры доказанной заранее и не вводить отдельные подгонки для каждого пола/ракурса. Планировка, свет и материалы при этом остаются теми же. Привязку media выполнять к существующему ID каталога (например, `glute-bridge`, `dumbbell-row`), а не создавать новые ID из подписей этой таблицы.

Финальные косметические настройки модалки после принятия шаблона: верхние интервалы сокращены суммарно на 20 px на desktop и на 12 px на узких экранах, активный ракурс выделен чуть более насыщенным зелёным. Размер видео и правая колонка сохраняются; повторный рендер для этих CSS-изменений не нужен.
...
Локальная доработка от 24.09.2026 сохраняет помещение и приведённые выше параметры света. Цвет дуба смещён к спокойному бежевому (`#bbad95`, основа процедурной карты `#e1d5bf`), снижен контраст древесных прожилок и фактуры штукатурки. Штукатурка — `#e3e2d9`. Это изменения материалов окружения, не текстур или PBR-материалов персонажей.
...
В локальной доработке ширина основной надписи уменьшена с `1.92` до `1.62`, боковой — с `1.52` до `1.28` единиц сцены (примерно на 16%). Цвет букв `#6c7968` снижает контраст относительно прежнего тёмно-зелёного. Позиции надписей сохранены; они по-прежнему являются материалом поверхности в помещении.
...
Параметры локальной серии: горизонтальный радиус камеры `4.35`, высота `0.12`, вертикальный FOV `28°`, общая точка прицеливания `[0, -0.18, 0]`. Позиции: спереди `[0, 0.12, 4.35]`, сбоку `[4.35, 0.12, 0]`, сзади `[0, 0.12, -4.35]`, 3/4 `[4.35/√2, 0.12, 4.35/√2]`. У обоих персонажей один набор камер. Снижение точки прицеливания с `-0.06` до `-0.18` добавляет безопасный отступ под обувью без изменения масштаба скелета, радиуса или FOV.

Проверка `tools/verify-squat-studio-framing.mjs` проецирует фактически деформированные вершины тела, одежды и обуви, а не только кости или bounding box нейтральной позы. Для двух персонажей и четырёх камер берётся по 72 момента, включая границы исходного клипа. В отчёте от 24.09.2026 все 576 выборок прошли: минимальный отступ от границы кадра — `6.22%`, предупреждений и ошибок нет. Результат: `artifacts/studio-quality-review/framing-report.json`. Это конечная выборка времени, а не математическое доказательство для всех промежуточных моментов и не проверка спортивной техники.
...
Рендер: `tools/render-squat-technique-review-browser.mjs`. Кодирование и подготовка poster: `tools/create-squat-release-videos.mjs`. WebM использует VP9 и единое квадратное разрешение `900×900`. Для исходного клипа длительностью `2.375 с` берутся 72 равномерные выборки времени: `t=i×2.375/72`, средняя частота — `30.315789 fps`. Последний кадр не дублирует первый, цикл возвращается к кадру 0.

MediaRecorder кодирует изображения, но его таймер даёт небольшие отклонения. `tools/webm-frame-timing.mjs` нормализует временные метки WebM по исходному клипу без повторного кодирования изображений, добавления или удаления кадров. Проверяются все 72 блока, длительность `2.375 с`, сохранность каждого сжатого кадра и PTS с погрешностью не более `0.5 мс` при масштабе WebM `1 мс`. Неизвестная структура контейнера отклоняется. Проверка релизных media читает фактические временные метки, а не только manifest. Ключи анимации остаются прежними. Poster берётся из начала соответствующего ролика.
...
После рендера обновляются cache revision и фактические хеши в существующих metadata/manifests. При сохранении планировки локальная доработка остаётся студией `telo365-functional-studio-v3`; её cache revision — `studio-v3-polish-local-20260924`. Успешная генерация файлов сама по себе не подтверждает визуальное качество или технику упражнения.
...
Базовая серия опубликована в выпуске `4496ddd`; фактическая проверка доставки сохранена в `artifacts/squat-release-review/deployment-4496ddd.json`. Состояние интерфейса, ограничения и список проверок описаны в [SQUAT_PRESENTATION_LOCAL_REVIEW.md](SQUAT_PRESENTATION_LOCAL_REVIEW.md). Инструкции по повторному рендеру сами по себе не разрешают публикацию новой серии.
