...
const legal = {
  ...
  privacy: { label: 'Политика конфиденциальности', title: 'Как мы обращаемся с данными', lead: 'Здесь описано, какие данные сохраняет TELO365 и как ими можно управлять.', sections: [{ title: 'Какие данные сохраняются', text: 'При регистрации сохраняются email, имя, цель и технические данные для входа. В кабинете хранятся только записи, которые вы добавляете сами: питание, тренировки, вес, привычки и покупки.' }, { title: 'Зачем они нужны', text: 'Данные нужны для работы личного кабинета, синхронизации между устройствами и восстановления доступа. Мы не продаём записи пользователей и не подключаем рекламные трекеры.' }, { title: 'Аналитика', text: 'Сервис ведёт только обезличенные суммарные счётчики посещений страниц и действий на сайте. Они не содержат имя, email, IP-адрес или профиль посетителя.' }, { title: 'Фото блюд', text: 'Если вы сами отправляете фото блюда на распознавание, оно передаётся OpenAI для подготовки ориентировочной оценки состава и КБЖУ. TELO365 не сохраняет такие фотографии. Проверьте оценку перед добавлением в дневник.' }, { title: 'Ваш контроль', text: 'В профиле можно выгрузить данные и удалить аккаунт. После удаления активные записи и сессии удаляются. Резервные копии хранятся ограниченный срок для восстановления после технического сбоя.' }] },
