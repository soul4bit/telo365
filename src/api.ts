...
export type MovementPattern='squat'|'hinge'|'horizontal_push'|'horizontal_pull'|'vertical_push'|'vertical_pull'|'carry'|'core'|'locomotion'|'mobility'
export type Exercise={id:string;name:string;movementPattern:MovementPattern;primaryMuscles:string[];secondaryMuscles:string[];difficulty:string;locations:string[];equipment:string[];cautionTags:string[];contraindicationTags:string[];contraindications:string[];alternatives:string[];animationKey:string;instructions:string[];defaultRestSeconds:number;techniqueTips:string[];media:ExerciseMedia;variants:string[];unit:'reps'|'seconds'}
...
export type PlannedExercise={exerciseId:string;sets:number;reps:number|null;repsMin?:number|null;repsMax?:number|null;durationSeconds:number|null;restSeconds:number;weight:number|null;targetRpe:number;targetEffort:string;order:number;notes:string;substitutionGroup:string;variants:string[];alternatives?:ExerciseAlternative[]}
export type TrainingDay={id:string;weekday:number;date:string;type:string;name:string;focus?:string;templateKey?:string;durationMinutes:number;intensity:'light'|'moderate'|'hard';recoveryDay:boolean;status:string;exercises:PlannedExercise[]}
export type TrainingWeek={id:string;weekStart:string;days:TrainingDay[]}
export type TrainingPlan={id:string;version?:number;status:'active'|'needs_review';source:Record<string,unknown>;safety:{restricted:boolean;cautious:boolean;intensityLimited:boolean;limitations:string[];blockedExerciseAreas?:string[];message:string};adjustment:{requestedDaysPerWeek:number;plannedDaysPerWeek:number;reduced:boolean;reasons:string[]};currentWeek:TrainingWeek;progression:{mode:'rule_based';nextReviewAfterSessions:number;feedbackStored?:boolean}}
export type CatalogItem = {id:string;kind:string;name:string;owner:string|null;kcal?:number;p?:number;f?:number;c?:number;source?:string;sample?:boolean;instructions?:string;image?:string;nutrition?:Snapshot;ingredients?:{foodId:string;grams:number}[];description?:string;minutes?:number;intensity?:'light'|'moderate'|'hard';muscle?:string;unit?:'reps'|'seconds';exercises?:{exerciseId:string;sets:number;reps:number;weight:number}[];movementPattern?:MovementPattern;primaryMuscles?:string[];secondaryMuscles?:string[];difficulty?:string;locations?:string[];equipment?:string[];cautionTags?:string[];contraindicationTags?:string[];contraindications?:string[];animationKey?:string;defaultRestSeconds?:number;techniqueTips?:string[];media?:ExerciseMedia;alternatives?:string[];variants?:string[]}
...
export type Workout={id:string;date:string;finished:boolean;data:{name:string;programId:string;planId?:string;trainingDayId?:string;intensity?:string;minutes?:number;startedAt?:string;exercises:WorkoutExercise[];feedback?:{rpe:number;perceivedDifficulty?:'easy'|'normal'|'hard';allSetsCompleted:boolean;painOrDiscomfort:boolean;discomfortArea?:string|null}|null;progressRecommendation?:{action:string;label:string;reason:string}|null}}
