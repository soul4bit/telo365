...
export function LegalPage({ page }: { page: 'about' | 'privacy' | 'consent' | 'terms' | 'contacts' }) {
...
function Footer() { return <footer className="marketing-footer"><a href="/"><Logo /></a><div className="marketing-footer-links"><a href="/about">О сервисе</a><a href="/privacy">Конфиденциальность</a><a href="/consent">Согласие на обработку данных</a><a href="/terms">Условия использования</a><a href="/contacts">Контакты</a><a href="/contacts">Поддержка</a></div><small>© {new Date().getFullYear()} TELO365</small></footer> }
...
const legal = {
  ...
  privacy: {
    ...
    sections: [
      ...
      { title: 'Отдельное согласие и контроль', text: 'Сведения о здоровье и физических ограничениях обрабатываются только после отдельного добровольного согласия пользователя. В профиле их можно удалить без удаления аккаунта: после этого сведения перестают использоваться для новых тренировочных рекомендаций. Не указывайте названия лекарственных препаратов, диагнозы и другие медицинские сведения, если они не нужны для адаптации физической нагрузки.' },
      { title: 'Кому могут передаваться данные', text: 'Сведения о здоровье и физических ограничениях не передаются в сервис распознавания фотографий и не отправляются во внешние AI/API. Они используются внутри TELO365 для формирования и адаптации тренировочного плана.' },
      ...
      { title: 'Ваш контроль', text: 'В профиле можно выгрузить данные, изменить стартовую анкету, удалить сведения о физических ограничениях или удалить аккаунт. После удаления аккаунта активные записи и сессии удаляются. Резервные копии хранятся до 14 дней для восстановления после технического сбоя.' }
    ]
  },
  consent: { label: 'Согласие на обработку данных', title: 'Согласие на обработку персональных данных', lead: 'Здесь описано, какое согласие вы подтверждаете при использовании TELO365.', sections: [{ title: 'Для чего нужно согласие', text: 'Согласие нужно, чтобы хранить и синхронизировать ваши записи в аккаунте: профиль, питание, тренировки, привычки, вес и покупки.' }, { title: 'Отдельное согласие на данные о здоровье', text: 'Если вы добровольно указываете сведения о физических ограничениях, мы запрашиваем отдельное добровольное согласие. Эти сведения нужны только для адаптации тренировок и могут быть удалены в профиле.' }, { title: 'Как отозвать согласие', text: 'Вы можете удалить сведения о физических ограничениях в профиле. После удаления они перестанут использоваться для новых тренировочных рекомендаций.' }] },
