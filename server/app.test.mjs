...
test('account and journal integration', async t=>{
  ...
  await t.test('onboarding persists a draft and applies a safe starter plan',async()=>{
    ...
    const draft={age:30,sex:'female',heightCm:168,weightKg:70,targetWeightKg:65,primaryGoal:'lose_weight',secondaryGoals:['move_more'],activityLevel:'light',averageSteps:5000,averageSleepHours:7,trainingDaysPerWeek:3,trainingDurationMinutes:35,trainingLocations:['home'],equipment:['dumbbells'],trainingExperience:'beginner',limitations:['none'],limitationNotes:'',doctorRestrictions:'',acutePainOrExerciseRestriction:false,acutePainNow:false,doctorExerciseRestriction:false,medicationConsideration:'no',mealsPerDay:4,cooking:'normal',foodBudget:5000,foodBudgetPeriod:'week',allergies:['\u043c\u043e\u043b\u043e\u043a\u043e'],excludedFoods:[],likedFoods:['eggs'],preferredStores:['shop'],dietType:'any',preferredTrainingDays:[1,3,5],preferredTrainingTime:'evening',habitPreferences:['water','sleep','steps','food','workouts']};
    ...
    const noConsent={...draft,limitations:['knees'],medicationConsideration:'yes',healthDataConsent:false};assert.equal((await req('/api/onboarding/complete','POST',{data:noConsent},'start')).status,400);const adapted={...draft,limitations:['knees','asthma','cardio'],areaStatusByArea:{knees:['pain','recovery']},recoveryTimingByArea:{knees:'one_to_six_months'},medicationConsideration:'yes',medicationGuidance:['pulse','pressure','avoid_high_intensity','avoid_exercises'],medicationNotes:'monitor pulse',healthDataConsent:true};const adaptedPlan=await req('/api/onboarding/complete','POST',{data:adapted},'start');assert.equal(adaptedPlan.status,200);assert.equal(adaptedPlan.value.plan.safety.avoidHighImpact,true);assert.equal(adaptedPlan.value.plan.safety.monitorIntensity,true);assert.equal(adaptedPlan.value.plan.safety.avoidHiit,true);assert.equal(adaptedPlan.value.plan.safety.reduceKneeLoad,true);assert.equal(adaptedPlan.value.plan.safety.avoidSpecificExercises,true);assert.equal(adaptedPlan.value.plan.safety.requiresAdaptedTraining,true);assert.equal(JSON.stringify(adaptedPlan.value.plan).includes('monitor pulse'),false);
    const cautious={...draft,acutePainNow:true,acutePainOrExerciseRestriction:false,healthDataConsent:true};assert.equal((await req('/api/onboarding/complete','POST',{data:cautious},'start')).status,200);assert.equal((await req('/api/onboarding','GET',undefined,'start')).value.plan.healthSafetyLevel,'cautious');const restricted={...draft,doctorExerciseRestriction:true,healthDataConsent:true};assert.equal((await req('/api/onboarding/complete','POST',{data:restricted},'start')).status,200);assert.equal((await req('/api/onboarding','GET',undefined,'start')).value.plan.healthSafetyLevel,'restricted');
  });
