...
Run:
  blender -b assets-work/telo-trainer-male-squat-animation-working.blend \
    -P tools/prepare-visual-squat-candidate-v3.py -- male
"""
import bpy
import json
import os
...
CONFIG = {
    "male": {
        ...
        "working": os.path.join(ROOT, "assets-work", "telo-trainer-male-squat-visual-v3-working.blend"),
        "model": os.path.join(ROOT, "public", "media", "exercises", "models", "telo-trainer-male-visual-v3-test.glb"),
        "renders": os.path.join(ROOT, "artifacts", "male-visual-v3-squat-keyframes"),
        "video": os.path.join(ROOT, "artifacts", "male-visual-v3-squat-preview.mp4"),
    ...
    "female": {
        ...
        "working": os.path.join(ROOT, "assets-work", "telo-trainer-female-squat-visual-v3-working.blend"),
        "model": os.path.join(ROOT, "public", "media", "exercises", "models", "telo-trainer-female-visual-v3-test.glb"),
        "renders": os.path.join(ROOT, "artifacts", "female-visual-v3-squat-keyframes"),
        "video": os.path.join(ROOT, "artifacts", "female-visual-v3-squat-preview.mp4"),
...
def make_surface_garment(name, predicate, mat, thickness=0.012):
    ...
    # The Human Base Mesh face selection is dense enough for a light geometry
    # smooth. It softens the saw-tooth outline without changing the body mesh,
    # rig, rest pose or copied skin weights.
    smoothing = garment.modifiers.new("TELO garment surface smoothing", "SMOOTH")
...
def shoes_predicate(point):
    return point.z <= 0.125 and point.y <= 0.10 and abs(point.x) <= 0.30
...
for name, predicate, mat, thickness in (
    ("TELO fitted sport top", shirt_predicate, top_mat, 0.006),
    ("TELO fitted sport bottoms", bottoms_predicate, bottom_mat, 0.005),
    ("TELO low-profile training shoes", shoes_predicate, shoe_mat, 0.006),
    ("TELO short hair cap", hair_predicate, hair_mat, 0.007),
):
...
body["teloClothingGeometry"] = ["fitted sport top", "fitted sport bottoms", "low-profile training shoes", "short hair cap"]
...
render("standing-side", 0, (4.5, -0.2, 1.42), (0.0, 0.0, 0.84))
render("standing-back", 0, (0.0, 4.5, 1.56), (0.0, 0.0, 0.88))
...
print("TELO_VISUAL_V3_CANDIDATE=" + json.dumps(report, ensure_ascii=False))
