...
> Доработка оформления от 24.09.2026 описана в [SQUAT_PRESENTATION_LOCAL_REVIEW.md](SQUAT_PRESENTATION_LOCAL_REVIEW.md). Она была проверена локально, после чего владелец отдельным сообщением разрешил commit, push и deploy видеоверсии. Статус `specialistTechniqueReview` остаётся `pending`; исходные GLB/FBX не публикуются.

## Выбранный вариант

К релизу подготовлен вариант **B: видеодемонстрация**. В пользовательском окне «Техника: Приседания» можно выбрать «Женщина» или «Мужчина», запустить/поставить на паузу видео, вернуть его к началу и переключить скорость 0.5×/1×. По умолчанию ролик запускается на скорости **1×** в ракурсе **3/4**; дополнительно доступны «Спереди», «Сбоку» и «Сзади». Интерьер зала и физическая надпись `TELO365.RU` входят в отрендеренное изображение; HTML/CSS watermark поверх видео не используется. При `prefers-reduced-motion` показывается poster выбранного ракурса без автозапуска; пользователь может запустить видео вручную.
...
Проверенные WebM должны отвечать HTTP 200, два test GLB — не HTTP 200. Затем откройте «Тренировки → Техника: Приседания», проверьте выбор персонажа, четыре ракурса одной студии, физическую надпись внутри видео, controls, reduced motion, mobile-прокрутку и fallback при недоступном WebM.
...
## Единая виртуальная студия

Серия из восьми WebM и соответствующих posters использует сцену `telo365-functional-studio-v3`, описанную в [TELO365_FUNCTIONAL_STUDIO_RENDER_SPEC.md](TELO365_FUNCTIONAL_STUDIO_RENDER_SPEC.md). Это одно помещение со светлыми тёплыми стенами, шалфейными панелями, дубовыми рейками, тёмным резиновым полом и постоянными зонами для стойки, гантелей, скамей, гирь и перекладин. Все камеры и оба персонажа используют одинаковую геометрию помещения и единый мягкий свет. Боковой и задний ракурсы показывают узнаваемое оборудование, а `3/4` остаётся основным.

Версию фактически подключённой серии задаёт `studio.currentId` в `trainer/squat-video-release.manifest.json`; она должна соответствовать сгенерированным файлам и результатам проверок. Комната и надпись `TELO365.RU` запечены в WebM/PNG; существующий фон нельзя заменить средствами CSS поверх ролика.

Обновление оформления включает повторный рендер восьми роликов и соответствующих posters, обновление cache revision и фактических хешей. Оно не меняет интерфейс модалки, original Mixamo Air Squat, ID `squat`, связанный с ним пользовательский прогресс или статус специалистской проверки. Исходные модели по-прежнему остаются локальными.

Контрольные кадры v3 для обоих персонажей во всех четырёх ракурсах находятся в `artifacts/studio-quality-review/`: `male-contact-sheet.png`, `female-contact-sheet.png` и отдельные `{male|female}/{standing|bottom}-{front|side|back|three-quarter}.png` размером `1200×1200`. Файл `capture-manifest.json` фиксирует точное время кадров и хеши. Повторный захват: `node tools/capture-studio-quality.mjs --port 4175 --avatar both --size 1200` при запущенном локальном Vite; полная последовательность рендера приведена в спецификации студии. Эти материалы подтверждают наличие проверяемых кадров, но не заменяют заключение специалиста: `specialistTechniqueReview` остаётся `pending`.
