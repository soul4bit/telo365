...
Текущая доработка выполняется только локально, без commit, push и deploy. Состояние интерфейса, ограничения и список проверок описаны в [SQUAT_PRESENTATION_LOCAL_REVIEW.md](SQUAT_PRESENTATION_LOCAL_REVIEW.md). Инструкции по воспроизведению рендера сами по себе не означают, что новая серия уже опубликована.
