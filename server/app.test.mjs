...
test('account and journal integration', async t=>{
  ...
  await t.test('onboarding persists a draft and applies a safe starter plan',async()=>{
    ...
    const cautious={...draft,acutePainNow:true,acutePainOrExerciseRestriction:false};assert.equal((await req('/api/onboarding/complete','POST',{data:cautious},'start')).status,200);assert.equal((await req('/api/onboarding','GET',undefined,'start')).value.plan.healthSafetyLevel,'cautious');const restricted={...draft,doctorExerciseRestriction:true};assert.equal((await req('/api/onboarding/complete','POST',{data:restricted},'start')).status,200);assert.equal((await req('/api/onboarding','GET',undefined,'start')).value.plan.healthSafetyLevel,'restricted');
  });
