...
async function finishOnboarding(page:any){
  ...
  await expect(page.getByRole('heading',{name:/\u0422\u0432\u043e\u0439 \u043f\u043b\u0430\u043d \u0433\u043e\u0442\u043e\u0432/})).toBeVisible({timeout:8000})
  await page.getByRole('button',{name:'\u041f\u043e\u0447\u0435\u043c\u0443 \u0442\u0430\u043a\u043e\u0439 \u043f\u043b\u0430\u043d?',exact:true}).click()
  await expect(page.getByText('\u041a\u0430\u043a \u043e\u0442\u0432\u0435\u0442\u044b \u043f\u043e\u0432\u043b\u0438\u044f\u043b\u0438 \u043d\u0430 \u043f\u043b\u0430\u043d',{exact:true})).toBeVisible()
  await expect(page.locator('.onboarding-training-adjustment')).toContainText('3')
  await page.locator('.onboarding-plan-inputs').locator('summary').click()
  await expect(page.locator('.onboarding-plan-inputs')).toHaveAttribute('open','')
  await page.locator('.onboarding-ready-habits').click()
  await expect(page.locator('.onboarding-habit-preview')).toBeVisible()
  await page.getByRole('button',{name:'\u041e\u0442\u043a\u0440\u044b\u0442\u044c \u043c\u043e\u0439 \u0434\u0435\u043d\u044c',exact:true}).click()
...
test('guest landing, registration and personal journal work across devices',async({page,browser},info)=>{
  ...
  const nutritionHero=page.locator('.nutrition-hero')
  await expect(nutritionHero).toBeVisible()
  const nutritionVideo=page.locator('.nutrition-hero-video')
  if(info.project.name==='mobile')await expect(nutritionVideo).toHaveCount(0)
  else{
    await expect(nutritionVideo).toHaveAttribute('poster','/media/nutrition-hero-poster.webp')
    await expect(nutritionVideo.locator('source')).toHaveAttribute('src','/media/nutrition-hero.mp4')
    await expect(nutritionVideo).toHaveJSProperty('autoplay',true)
    await expect(nutritionVideo).toHaveJSProperty('muted',true)
    await expect(nutritionVideo).toHaveJSProperty('playsInline',true)
    await page.emulateMedia({reducedMotion:'reduce'})
    await expect(nutritionVideo).toHaveCount(0)
    await page.emulateMedia({reducedMotion:'no-preference'})
    await expect(nutritionVideo).toHaveCount(1)
    await nutritionVideo.evaluate(video=>video.dispatchEvent(new Event('error')))
    await expect(nutritionVideo).toHaveCSS('display','none')
  }
