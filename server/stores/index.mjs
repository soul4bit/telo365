...
import { STORE_CHAINS, STORE_CHAIN_SLUGS, SHOPPING_PRIORITIES } from './types.mjs';
import { aggregateIngredients } from './services.mjs';

const preferenceFor=(db,userId)=>{const row=db.prepare('SELECT * FROM food_preferences WHERE user_id=?').get(userId);return row?{preferredStores:JSON.parse(row.preferred_store_chains),shoppingPriority:row.shopping_priority,budgetMinor:row.food_budget_unlimited?null:row.weekly_food_budget_minor}:{};};
...
export async function stores(ctx) {
  ...
  if((path==='/api/nutrition/estimate-cost'||path==='/api/nutrition/shopping-list')&&method==='POST'){
    const list=requirements(body.requirements),preferences=preferenceFor(db,user.id),priority=body.shoppingPriority===undefined?preferences.shoppingPriority:body.shoppingPriority;
    if(!SHOPPING_PRIORITIES.includes(priority||'no_preference'))fail(400,'Проверьте приоритет');
    const result=storeCatalog.estimate(list,{...preferences,shoppingPriority:priority||'no_preference',preferredStores:chainValues(body.preferredStores??preferences.preferredStores)});
