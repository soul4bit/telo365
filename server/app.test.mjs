...
test('account and journal integration', async t=>{
  ...
  await t.test('onboarding persists a draft and applies a safe starter plan',async()=>{
    ...
    const single={...draft,currentTrainingDays:0,trainingDaysPerWeek:1};const singlePlan=await req('/api/onboarding/complete','POST',{data:single},'start');assert.equal(singlePlan.status,200);assert.equal(singlePlan.value.plan.trainingDaysPerWeek,1);
