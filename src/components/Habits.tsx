...
export default function Habits({data,mutate,busy,full=false}:{data:State;mutate:Mutate;busy:boolean;full?:boolean}) {
  ...
  if(!full)return <><CompactHabits data={data} done={done} values={values} busy={busy} perform={perform} add={openNew} edit={openEdit}/>{editor&&<HabitEditor habit={current} close={()=>{setEditor(null);setCurrent(null)}} remove={async()=>{if(!current)return;await perform(`/api/habits/${current.id}`,'DELETE',{});setEditor(null)}} mutate={mutate} busy={busy}/>}</>;return <section className={`card habits-panel ${full?'habits-panel-full':''}`}><div className="habits-heading"><div><span className="eyebrow">ТВОЙ РИТМ</span><h2>{full?'Привычки':'Привычки за день'}</h2><p>{data.date>data.today?'Отметки станут доступны в этот день':active.length?`Сегодня ${completed} из ${active.length} запланировано`:'На сегодня нет запланированных привычек'}</p></div><button className="soft-button" onClick={openNew}><Plus size={16}/>Новая</button></div>{full&&<><div className="habit-hero-stats"><div><span className="round-icon green"><Flame size={19}/></span><strong>{data.habits.length?Math.max(...data.habits.map(habit=>streak(habit,done,selected))):0}</strong><small>лучшая серия сейчас</small></div><div><span className="round-icon yellow"><Target size={19}/></span><strong>{completed}/{active.length}</strong><small>сделано сегодня</small></div><div><span className="round-icon purple"><Sparkles size={19}/></span><strong>{data.habits.length}</strong><small>в твоём ритме</small></div></div><div className="habit-templates"><span>Быстро добавить:</span>{templates.filter(template=>!data.habits.some(habit=>habit.name===template[1])).slice(0,4).map(([icon,name,schedule,weeklyTarget])=><button key={name} disabled={busy} onClick={()=>perform('/api/habits','POST',{name,icon,schedule,weeklyTarget})}>{icon} {name}</button>)}</div></>}{!data.habits.length?<Empty action={openNew} label="Создать привычку">Начни с одного маленького действия. Его легко заметить, а потом повторить.</Empty>:<div className="habit-cards">{cards.map(habit=>{const isDone=done.has(keyFor(habit,data.date)),isScheduled=scheduled(habit,data.date),currentStreak=streak(habit,done,selected),weekDone=week.filter(date=>scheduled(habit,date)&&done.has(keyFor(habit,date))).length;if(habit.tracking==='hydration')return <HydrationCard key={habit.id} habit={habit} value={values.get(keyFor(habit,data.date))} date={data.date} today={data.today} busy={busy} perform={perform} edit={()=>openEdit(habit)} catalog={data.catalog}/>;if(habit.tracking==='counter')return <CounterCard key={habit.id} habit={habit} value={values.get(keyFor(habit,data.date))?.value||0} date={data.date} today={data.today} busy={busy} perform={perform} edit={()=>openEdit(habit)}/>;return <article className={`habit-card ${isDone?'is-done':''}`} key={habit.id}><div className="habit-card-top"><button className="habit-toggle" aria-label={`${habit.name}: ${isDone?'выполнено':'не выполнено'}`} aria-pressed={isDone} disabled={busy||data.date>data.today||!isScheduled} onClick={()=>perform('/api/marks','PUT',{habitId:habit.id,date:data.date,done:!isDone})}><span className="habit-icon">{habit.icon||'✨'}</span><span><strong>{habit.name}</strong><small>{isScheduled?scheduleLabels[habit.schedule]||scheduleLabels.daily:'Сегодня не запланировано'}</small></span><span className="habit-check-big">{isDone?<Check size={18}/>:null}</span></button><button className="habit-settings" aria-label={`Настроить привычку ${habit.name}`} onClick={()=>openEdit(habit)}><Pencil size={15}/></button></div><div className="habit-calendar" aria-label={`Последние 14 дней: ${habit.name}`}>{Array.from({length:14},(_,index)=>plusDays(data.date,-13+index)).map(date=>{const isHabitDay=scheduled(habit,date),marked=done.has(keyFor(habit,date));return <span className={!isHabitDay?'skip':marked?'done':''} title={`${date}: ${!isHabitDay?'не запланировано':marked?'выполнено':'нет отметки'}`} key={date}>{atNoon(date).getUTCDate()}</span>})}</div><div className="habit-card-foot"><span><Flame size={14}/>{currentStreak} {ending(currentStreak,'день','дня','дней')}</span><span>{weekDone}/{habit.weeklyTarget||7} на этой неделе</span></div></article>})}</div>}<ErrorMessage error={error}/>{editor&&<HabitEditor habit={current} close={()=>{setEditor(null);setCurrent(null)}} remove={async()=>{if(!current)return;await perform(`/api/habits/${current.id}`,'DELETE',{});setEditor(null)}} mutate={mutate} busy={busy}/>}</section>
}

function CompactHabits({data,done,values,busy,perform,add,edit}:{data:State;done:Set<string>;values:Map<string,State['habitValues'][number]>;busy:boolean;perform:(path:string,method:string,body?:unknown)=>Promise<void>;add:()=>void;edit:(habit:Habit)=>void}) {
  const habits=data.habits.filter(habit=>scheduled(habit,data.date)).slice(0,5),completed=habits.filter(habit=>done.has(keyFor(habit,data.date))).length,water=habits.find(habit=>habit.tracking==='hydration'),waterValue=water?values.get(keyFor(water,data.date)):undefined,waterTotal=waterValue?.value||0,waterTarget=water?.target||2000,waterPercent=Math.min(100,Math.round(waterTotal/waterTarget*100))
  return <section className="card habits-panel home-habits"><div className="habits-heading"><div><span className="eyebrow">ТВОЙ РИТМ</span><h2>Привычки за день</h2><p>Сегодня {completed} из {habits.length} · {habits.length?Math.round(completed/habits.length*100):0}%</p></div><button className="soft-button" onClick={add}><Plus size={16}/>Новая</button></div><div className="compact-habit-list">{water&&<article className={`compact-water ${waterPercent>=100?'is-done':''}`}><div className="compact-water-icon"><Droplets size={19}/></div><div className="compact-water-copy"><strong>{water.name}</strong><span>{waterPercent>=100?`Цель выполнена · ${waterTotal.toLocaleString('ru-RU')} мл ✓`:`${waterTotal.toLocaleString('ru-RU')} / ${waterTarget.toLocaleString('ru-RU')} мл`}</span><i><b style={{width:`${waterPercent}%`}}/></i></div>{waterPercent<100&&<span className="compact-water-actions"><button className="compact-water-add" disabled={busy||data.date>data.today} onClick={()=>perform('/api/habits/value','PUT',{habitId:water.id,date:data.date,delta:250,kind:'water'})}>+250</button><button className="compact-water-add secondary" disabled={busy||data.date>data.today} onClick={()=>perform('/api/habits/value','PUT',{habitId:water.id,date:data.date,delta:500,kind:'water'})}>+500</button></span>}<button className="habit-settings" aria-label={`Настроить привычку ${water.name}`} onClick={()=>edit(water)}><Pencil size={15}/></button><span className="compact-check" aria-label={`${water.name}: ${waterPercent>=100?'выполнено':'не выполнено'}`} aria-pressed={waterPercent>=100}><Check size={15}/></span></article>}{habits.filter(habit=>habit!==water).map(habit=>{const isDone=done.has(keyFor(habit,data.date));return <article className={`compact-habit-row ${isDone?'is-done':''}`} key={habit.id}><button className="compact-habit-main" disabled={busy||data.date>data.today} onClick={()=>perform('/api/marks','PUT',{habitId:habit.id,date:data.date,done:!isDone})}><span className="habit-icon">{habit.icon||'✨'}</span><span><strong>{habit.name}</strong><small>{habit.tracking==='counter'?`${values.get(keyFor(habit,data.date))?.value||0} / ${habit.target} ${habit.unit}`:isDone?'Выполнено':'Не выполнено'}</small></span></button><button className="habit-settings" aria-label={`Настроить привычку ${habit.name}`} onClick={()=>edit(habit)}><Pencil size={15}/></button><button className="compact-check" aria-label={`${habit.name}: ${isDone?'выполнено':'не выполнено'}`} aria-pressed={isDone} disabled={busy||data.date>data.today} onClick={()=>perform('/api/marks','PUT',{habitId:habit.id,date:data.date,done:!isDone})}><Check size={15}/></button></article>})}</div><p className="compact-habits-foot">Полная история и настройки — в разделе «Привычки».</p></section>
}
...
function HydrationCard({habit,value,date,today,busy,perform,edit,catalog}:{habit:Habit;value:State['habitValues'][number]|undefined;date:string;today:string;busy:boolean;perform:(path:string,method:string,body?:unknown)=>Promise<void>;edit:()=>void;catalog:State['catalog']}) {
  ...
  const nutritionId=({lemonade:'drink-lemonade',juice:'drink-juice',milk:'drink-milk'} as Partial<Record<DrinkKind,string>>)[kind]
  async function addWithNutrition() { await change(amount); if(nutritionId&&catalog.some(item=>item.id===nutritionId)) await perform('/api/meals','PUT',{id:crypto.randomUUID(),date,slot:'Напитки',catalogId:nutritionId,servings:amount/100,eaten:true}) }
  ...
  return <article className={`habit-card hydration-card ${percent>=100?'is-done':''} ${pulse?'is-pulsing':''}`}>
    ...
    <div className="hydration-picker">
      ...
      <button className="hydration-add" disabled={busy||date>today} onClick={()=>change(amount)}><Plus size={16}/>Добавить</button>{nutritionId&&<button className="hydration-food" disabled={busy||date>today} onClick={addWithNutrition}>+ БЖУ</button>}
