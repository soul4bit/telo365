...
test('account and journal integration', async t=>{
  ...
  await t.test('workout restart, per-set data, completion and ownership',async()=>{
    ...
    workout=(await req('/api/state')).value.workouts[0];assert.equal(workout.finished,true);assert.equal(workout.data.feedback.rpe,6);assert.equal(workout.data.feedback.perceivedDifficulty,'normal');
