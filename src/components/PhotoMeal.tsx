...
export default function PhotoMeal({mutate,busy}:{mutate:Mutate;busy:boolean}) {
  ...
  return <section className="card photo-meal"><div className="photo-meal-heading"><span className="round-icon green"><ScanLine/></span><div><h2>Оценить блюдо по фото</h2><p>Снимите тарелку — покажем ориентировочные КБЖУ и состав.</p></div></div><label className="photo-picker"><input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" onChange={event=>choose(event.currentTarget.files?.[0])}/><Upload size={17}/><span>{image?'Выбрать другое фото':'Выбрать фото'}</span></label>{image&&<div className="photo-preview"><img src={image} alt="Выбранное блюдо"/><button className="primary-button" disabled={loading||busy} onClick={analyze}>{loading?<LoaderCircle className="spin" size={17}/>:<Camera size={17}/>}Распознать блюдо</button></div>}<p className="form-note">Фото передаётся OpenAI только для этого запроса и не сохраняется в TELO365. Оценка может ошибаться — сверяйте порцию и данные с упаковкой.</p><ErrorMessage error={error}/>{estimate&&<div className="photo-result"><div className="photo-result-title"><div><small>{confidence[estimate.confidence]}</small><h3>{estimate.name}</h3></div><strong>{fmt(estimate.kcal)} <small>ккал</small></strong></div><div className="photo-macros"><span>Б <b>{fmt(estimate.p)} г</b></span><span>Ж <b>{fmt(estimate.f)} г</b></span><span>У <b>{fmt(estimate.c)} г</b></span></div><p>{estimate.components.map(item=>`${item.name} · ${fmt(item.grams)} г`).join(', ')}</p>{saved?<p className="photo-saved"><Check size={15}/>Добавлено в личный каталог. Найдите блюдо во вкладке «Продукты», чтобы внести его в дневник.</p>:<button className="soft-button" disabled={loading||busy} onClick={save}><Check size={15}/>Сохранить как свой продукт</button>}</div>}</section>
}
